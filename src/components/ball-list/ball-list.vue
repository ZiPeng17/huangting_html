<template>
  <table class="ball-list" cellspacing="1" cellpadding="0" :width="width" border="0">
    ...
    <tr class="ball_tr_H" v-for="tab_item in info.data" ref="list">
      <td :width="width/5" class="td_1" v-if="words.indexOf(tab_item.no) > -1">{{tab_item.no}}</td>
      <td :width="width/5" v-else :class="'No_'+tab_item.no"></td>
      <td class="td_2">
      ...
      <td class="td_3" v-show="type_index == 2">
        <input class="inp1" onblur="this.className='inp1';AddBet(3008);" id="jeuM_3008"  onfocus="this.className='inp1m'" onkeypress="digitOnly(event)" maxlength="9" size="5" name="jeuM_3008">
      </td>
    </tr>
  </table>
</template>
